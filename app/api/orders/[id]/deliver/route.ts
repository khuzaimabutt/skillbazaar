...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: order } = await admin.from("orders").select("seller_id").eq("id", params.id).single();
  if (!order || order.seller_id !== user.id) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
